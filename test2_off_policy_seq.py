...
class Experience(object):
    ...
    def is_ready(self):
        return len(self._frames) >= self._history_size / 4
...
exp_rep_size = 2000
exp_rep = Experience(exp_rep_size)
batch_size = 32
gamma = 0.99
...
class Agent():
    ...
    def run_n_steps(self):
        ...
        for _ in range(self.t_max):
            ...
            exp_rep.add_frame(Transition_pr(
...
class Coordinator():
    ...
    def run(self):
        ...
        for i in shuf_arr:
            ...
            ### replay
            if exp_rep.is_ready():
                transitions = exp_rep.sample_sequence(20)
                ...
                for transition in transitions[::-1]:
                    ...
                    probs_old.append(transition.prob)
                
                probs_new = self.sess.run(self.model_net.probs_pi, {self.model_net.states: states})
                probs_new = probs_new[range(len(transitions)), actions]

                value_targets *= probs_new / probs_old
