...
class Coordinator():
    def __init__(self, sess, num_agents, envs, state_pr, model_net, gamma, tau, t_max, num_epochs, bsize):
        self.sess = sess
    ...
    def update(self, transitions_batch):
        ...
        for i in range(self.num_agents):
            ...
            R, value_next, gae = 0., 0., 0.
            if not transitions[-1].done:
                value_next = self._value_net_predict(transitions[-1].next_state)
                R = value_next
            ...
            for transition in transitions[::-1]:
                ...
                value_cur = self._value_net_predict(transition.state)
                delta_t = transition.reward + self.gamma * value_next - value_cur
...
if __name__ == '__main__':
    ...
    state_pr = StateProcessor(args.imsize)
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        coord = Coordinator(sess=sess, num_agents=args.num_agents, envs=envs, state_pr=state_pr, model_net=model, \
            gamma=args.gamma, tau=args.tau, t_max=args.t_max, num_epochs=args.num_epochs, bsize=args.bsize)
